...
class GameConfig(BaseModel):
    ...
    rows: int = Field(default=6, ge=4, le=10, description="Number of rows in the game board")
    cols: int = Field(default=7, ge=4, le=10, description="Number of columns in the game board")
    win_length: int = Field(default=4, ge=3, le=8, description="Number of consecutive pieces needed to win")
    ...
    def validate_win_length(cls, v, info):
        ...
        if info.data:
            ...
            if v > max_dimension:
                raise ValueError(f"win_length ({v}) cannot be greater than max board dimension ({max_dimension})")
        return v
